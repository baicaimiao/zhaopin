...
import { 
  ...
  Activity, UserCheck, UserX, Eye, Download, Radar, Target
} from 'lucide-react';
import { parseResumeData, generateInterviewQuestions, generateComprehensiveFitAnalysis } from '../services/geminiService';
...
const EditableCell: React.FC<{ label: string; value: string; onSave: (val: string) => void; className?: string; multiline?: boolean; }> = ({ label, value, onSave, className, multiline = false }) => { const [isEditing, setIsEditing] = useState(false); const [tempValue, setTempValue] = useState(value); const inputRef = useRef<HTMLInputElement | HTMLTextAreaElement>(null); useEffect(() => { if (isEditing) { inputRef.current?.focus(); if (multiline && inputRef.current) { inputRef.current.setSelectionRange(tempValue.length, tempValue.length); } } else { setTempValue(value); } }, [isEditing, value, multiline, tempValue.length]); const handleBlur = () => { setIsEditing(false); if (tempValue !== value) onSave(tempValue); }; const handleKeyDown = (e: React.KeyboardEvent) => { if (e.key === 'Enter') { if (multiline) { if (!e.shiftKey) { e.preventDefault(); handleBlur(); } } else { handleBlur(); } } if (e.key === 'Escape') { setTempValue(value); setIsEditing(false); } }; return ( <div className={`group flex flex-col px-3 py-1.5 border-r border-b border-slate-100 min-h-[46px] transition-all hover:bg-indigo-50/30 relative ${className}`} onDoubleClick={() => setIsEditing(true)}> <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0 flex justify-between overflow-hidden"> <span className="truncate">{label}</span> <span className="opacity-0 group-hover:opacity-100 text-[8px] text-indigo-400 font-normal normal-case transition-opacity cursor-pointer shrink-0 ml-1">编辑</span> </span> {isEditing ? ( multiline ? ( <textarea ref={inputRef as React.RefObject<HTMLTextAreaElement>} className="w-full h-full min-h-[32px] bg-white border border-indigo-400 rounded px-1.5 py-0.5 text-xs font-bold text-slate-900 outline-none shadow-lg resize-none leading-tight" value={tempValue} onChange={(e) => setTempValue(e.target.value)} onBlur={handleBlur} onKeyDown={handleKeyDown} /> ) : ( <input ref={inputRef as React.RefObject<HTMLInputElement>} className="w-full bg-white border border-indigo-400 rounded px-1.5 py-0.5 text-xs font-bold text-slate-900 outline-none shadow-lg" value={tempValue} onChange={(e) => setTempValue(e.target.value)} onBlur={handleBlur} onKeyDown={handleKeyDown} /> ) ) : ( <div className={`text-xs font-bold leading-tight break-words whitespace-pre-wrap mt-0.5 ${value ? 'text-slate-800' : 'text-slate-300 italic'}`}> {value || '未填写'} </div> )} </div> ); };
...
const CandidateManagement: React.FC<Props> = ({ candidates, jobs, personas, initialFilterJobId, onAddCandidate, onUpdateCandidate }) => {
  ...
  const handleAnalyzeFit = async () => {
    ...
    try {
      const analysis = await generateComprehensiveFitAnalysis(
        candidateJob.title,
        jobPersona.description + (jobPersona.requirements?.join(' ') || ''),
        selectedCandidate.name,
        selectedCandidate.fullResumeText || '',
        selectedCandidate.interviews
      );
      await onUpdateCandidate({ ...selectedCandidate, fitAnalysis: analysis });
    } catch (e) {
      console.error(e);
      alert('分析生成失败，请重试');
    } finally {
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-12 custom-scrollbar bg-slate-50/30">
               ...
               <section className="scroll-mt-8">
                 <div className="flex items-center space-x-2 mb-6">
                    ...
                    <h4 className="text-[10px] font-black text-indigo-900 uppercase tracking-[0.2em] flex items-center">
                        <Target size={14} className="mr-2" />
                        AI 全维度人岗匹配雷达 (AI Fit Radar)
                    </h4>
                 </div>
                    ...
                    {!selectedCandidate.fitAnalysis ? (
                       <div className="text-center py-10 relative z-10">
                          <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center mx-auto mb-4 backdrop-blur-sm border border-white/10">
                             <Radar size={32} className="text-indigo-300" />
                          </div>
                          <h3 className="text-xl font-bold mb-2">启动智能匹配分析</h3>
                          <p className="text-white/60 text-sm mb-6 max-w-md mx-auto leading-relaxed">
                             AI 将综合分析 <span className="text-white font-bold">简历画像</span> + <span className="text-white font-bold">历史面试录音</span> + <span className="text-white font-bold">岗位标准</span>，为您生成一份决策级的匹配度报告。
                          </p>
                          <button 
                            onClick={handleAnalyzeFit} 
                            disabled={isAnalyzingFit}
                            className="px-8 py-3 bg-white text-indigo-900 rounded-xl text-sm font-black hover:bg-indigo-50 transition-all shadow-lg shadow-white/10 disabled:opacity-70 flex items-center mx-auto space-x-2"
                          >
                             {isAnalyzingFit ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
                             <span>{isAnalyzingFit ? '深度分析中 (约15秒)...' : '开始全维度诊断'}</span>
                          </button>
                       </div>
                    ) : (
                       <div className="relative z-10">
                          <div className="flex justify-between items-start mb-6 border-b border-white/10 pb-6">
                             <div>
                                ...
                                <p className="text-white/50 text-xs mt-1 font-mono uppercase tracking-widest">Powered by Gemini 3 Pro • 基于简历 & 面试全流程</p>
                             </div>
                             <button 
                               onClick={handleAnalyzeFit} 
                               disabled={isAnalyzingFit}
                               className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-xs font-bold text-white transition-all flex items-center border border-white/10"
                             >
                                {isAnalyzingFit ? <Loader2 size={12} className="animate-spin mr-1" /> : <Radar size={12} className="mr-1" />}
                                {isAnalyzingFit ? '更新中...' : '重新分析'}
                             </button>
                          </div>
                          
                          <div className="bg-white/5 rounded-2xl p-6 border border-white/10 text-sm leading-relaxed text-indigo-50 whitespace-pre-wrap font-medium shadow-inner max-h-[500px] overflow-y-auto custom-scrollbar">
                             {selectedCandidate.fitAnalysis}
                          </div>
                       </div>
                    )}
