  ...
  if (isLoading) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-slate-50 space-y-4">
        ...
        <p className="text-slate-500 font-bold">正在连接 MemFire Cloud 云端数据...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col p-6 space-y-8">
        ...
        <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-4 rounded-xl text-white shadow-lg shadow-indigo-100">
          <div className="flex items-center space-x-2 mb-2">
            <Zap size={14} className="fill-current" />
            <p className="text-xs font-bold uppercase">AI 深度引擎</p>
          </div>
          <p className="text-[10px] opacity-90 leading-relaxed">已连接云端数据库，数据实时同步</p>
        </div>
      </aside>
